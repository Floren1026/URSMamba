...
parser.add_argument('--channel_secret', type=int, default=4, help='1: gray; 3: color')
...
def main():
    ...
    ##################  Model initialize  ##################
    # Hnet = ConNet_new(in_chans=args.channel_cover+args.channel_secret, out_chans=args.channel_cover, imgsize=args.imageSize, hidden_dim=64, depths=[4, 4, 4, 4], mode='ssdf', use_att=True)
    # Rnet = RevNet_new(in_chans=args.channel_cover, out_chans=args.channel_secret, hidden_dim=64, depths=[4, 4, 4, 4])
    ...
    if args.train_continue:
        checkpoint_h = f"./training_MSI/{args.Model_dir}/checkPoints/checkpointH.pt"
        checkpoint_r = f"./training_MSI/{args.Model_dir}/checkPoints/checkpointR.pt"
    ...
    # write the networks
    writer = SummaryWriter(log_dir='runs_MSI/' + args.experiment_dir)


    ##################  Datasets  ##################
    COVER_DIR = '/home/datasets/imagenet'
    SECRET_DIR = '/home/datasets/multispectral/moscow'
    train_cover = os.path.join(COVER_DIR, 'train')
    train_secret = os.path.join(SECRET_DIR, 'train')
    val_cover = os.path.join(COVER_DIR, 'validation')
    val_secret = os.path.join(SECRET_DIR, 'val')

    batch_size_C=args.batch_stegs*args.num_cover
    batch_size_S=args.batch_stegs*args.num_secret

    train_loader_cover = load_data_rgb(args, train_cover, batch_size_C)
    train_loader_secret = load_data_multi(args, train_secret, batch_size_S)
    val_loader_cover = load_data_rgb(args, val_cover, batch_size_C)
    val_loader_secret = load_data_multi(args, val_secret, batch_size_S)

    args.train_size = min(len(train_loader_cover), len(train_loader_secret))
    args.val_size = min(len(val_loader_cover), len(val_loader_secret))
...
def train(trainLoader, epoch, Hnet, Rnet, criterion):
    ...
    for i, ((cover_img, cover_target), (secret_img)) in enumerate(trainLoader, 0):
        ...
        log = '[%d/%d][%d/%d]\tLoss_H: %.6f Loss_R: %.6f L1_H_diff: %.4f L1_R_diff: %.4f Loss_sum: %.6f \tdatatime: %.6f \tbatchtime: %.6f' % (
            epoch, args.epochs, i, args.train_size,
        ...
        if i % args.resultPicFrequency == 0:
            save_pic_multi(args, cover_imgv, steg_img.data, secret_imgv_r, rev_img.data, epoch, i, args.trainpics)
...
def validation(valLoader, epoch, Hnet, Rnet, criterion):
    ...
    with torch.no_grad():
        ...
        for i, ((cover_img, cover_target), (secret_img)) in enumerate(valLoader, 0):
            ...
            log = '[%d/%d][%d/%d]\tLoss_H: %.6f Loss_R: %.6f L1_H_diff: %.4f L1_R_diff: %.4f Loss_sum: %.6f' % (
                epoch, args.epochs, i, args.val_size, Hlosses.val, Rlosses.val, Hdiff.val, Rdiff.val, TotalLosses.val)
            if i % args.log_freq == 0:
                print(log)

            if i == 0:
                save_pic_multi(args, cover_imgv, steg_img.data, secret_imgv_r, rev_img.data, epoch, i, args.validationpics)
